...
public:
        ...
        const int res_x, const int res_y, const int f_width, const int f_height)
    {
        ...
        for (int j = 1; j < _t-1; ++j) {
      
            _pos.push_back(Vec2f(_l, j + 0.25));
            _pos.push_back(Vec2f(_l, j + 0.75));
      
            _pos.push_back(Vec2f(_r, j + 0.25));
            _pos.push_back(Vec2f(_r, j + 0.75));
        ...
        _pos.push_back(Vec2f(0.25, 0.25));
        _pos.push_back(Vec2f(0.75, 0.25));
        _pos.push_back(Vec2f(0.25, 0.75));
      
        _pos.push_back(Vec2f(0.25, _t));
        _pos.push_back(Vec2f(0.75, _t));
        _pos.push_back(Vec2f(0.25, _t-0.5));
      
        _pos.push_back(Vec2f(_r, _t));
        _pos.push_back(Vec2f(_r-0.5, _t));
        _pos.push_back(Vec2f(_r, _t-0.5));
      
        _pos.push_back(Vec2f(_r, 0.25));
        _pos.push_back(Vec2f(_r-0.5, 0.25));
        _pos.push_back(Vec2f(_r, 0.75));
        ...
        isBoundary = std::vector<bool>(_pos.size(), false);
        for (int i = 0; i < b; ++i) {
            isBoundary[i] = true;
    ...
    void buildNeighbor()
    {
        ...
        for (tIndex i = 0; i < particleCount(); i++) {
            ...
            tIndex x = floor(_pos[i].x);
            tIndex y = floor(_pos[i].y);

            tIndex cell = idx1d(x, y);

            _pidxInGrid[cell].push_back(i); //mmi mette l indice
            //della particella (i) dentro alla cella 
            //tutte le particelle nella stessa cella x sono in pidxingrid[x]
    ...
    std::vector<tIndex> getNeighbours(int x, int y) {
        std::vector<tIndex> neighbourcellss;
        neighbourcellss.push_back(idx1d(x, y));
        x > 0 ? neighbourcellss.push_back(idx1d(x - 1, y)) : void();
        y > 0 ? neighbourcellss.push_back(idx1d(x, y - 1)) : void();
        x < _resX - 1 ? neighbourcellss.push_back(idx1d(x + 1, y)) : void();
        y < _resY - 1 ? neighbourcellss.push_back(idx1d(x, y + 1)) : void();
        x > 0 && y > 0 ? neighbourcellss.push_back(idx1d(x - 1, y - 1)) : void();
        x > 0 && y < _resY - 1 ? neighbourcellss.push_back(idx1d(x - 1, y + 1)) : void();
        x < _resX - 1 && y > 0 ? neighbourcellss.push_back(idx1d(x + 1, y - 1)) : void();
        x < _resX - 1 && y < _resY - 1 ? neighbourcellss.push_back(idx1d(x + 1, y + 1)) : void();
        return neighbourcellss;
    }
    ...
    void computeDensity()
    {
        ...
        for (tIndex i = 0; i < _pos.size(); ++i) {
            ...
            _d[i] = 0.0f;
            std::vector<tIndex> neighbourcellss = getNeighbours(floor(_pos[i].x), floor(_pos[i].y));
            for (auto &cell : neighbourcellss) {
                for (auto &j : _pidxInGrid[cell]) {
                    _d[i] += _m0 * _kernel.w(_pos[i] - _pos[j]);
                }
            }
        }
    }
   ...
   void applyPressureForce()
    ...
    //              for (auto& j : _pidxInGrid[cell]) {
    //                  if (i != j) {
    //                      _acc[i] -= _m0 * _kernel.grad_w(_pos[i] - _pos[j]) * (_p[i] / pow(_d[i], 2) + _p[j] / pow(_d[j], 2)); //Real w(const Vec2f &rij) const { return f(rij.length()); }
    //                  }
    //              }
    //          }
    //          //   _acc[i] *= _m0; //questo è mi e gli altri erano mj
    //      }
    //  }                
    }
    ...
    //             for (auto& j : _pidxInGrid[cell]) {
    //                 if (i != j) {
    //                     _acc[i] += 2 * _nu * (_m0 / _d[j]) * (_vel[i] - _vel[j]) * (_kernel.w(_pos[i] - _pos[j])) * (_pos[i] - _pos[j]) / ((_pos[i] - _pos[j]) * (_pos[i] - _pos[j]) + 0.01 * pow(_h, 2));
    //                 }
    //             }
    //         }
    //         //   _acc[i] *= 2 * _nu * _m0; //questo è mi e gli altri erano mj
    //     }
    // }
    }
  ...
  Real _sr = 2e0 * _h;          // supporting radius
  Vec2f _g;                     // gravity

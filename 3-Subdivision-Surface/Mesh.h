...
public:
  ...
  void addNoise() {
      ...
      for (unsigned int v = 0; v < _vertexPositions.size(); ++v) {
          ...
          _vertexPositions[v]+= ((rand() % 100) - 50) * 0.001;
          ...
          for (unsigned int v = 0; v < _vertexPositions.size(); ++v) {
             ...
             for (auto it = trianglesOnEdge[v].begin(); it != trianglesOnEdge[v].end(); ++it) {
                 ...
                 if (it->second == 2) {
                      for (auto i = neighboringVertices[v].begin(); i != neighboringVertices[v].end(); ++i) {
                       //  glm::vec3 e_ij(0.0f);
                       //  //glm::vec3 e_ij = _vertexPositions[*i] - _vertexPositions[v];
                       //  if (i == neighboringVertices[v].begin()) {
                       //      i = neighboringVertices[v].end();
                       //
                       //      e_ij = _vertexPositions[*i] - _vertexPositions[v];
                       //  }
                       //  else {
                       //      i--;
                       //
                       ...
                       //
                       // ++i;
                       // if (i == neighboringVertices[v].end()) {
                       //     i = neighboringVertices[v].begin();
                       // }
                       // 
                       // glm::vec3 e_ik = _vertexPositions[*i] - _vertexPositions[v];
                          ...
                          glm::vec3 e_ijp = _vertexPositions[i_prev] - _vertexPositions[v]; //i,j-1
                          ...
                          glm::vec3 e_jjm = _vertexPositions[i_next] - _vertexPositions[i_next]; //j,j+1
                          float cot_beta = 1.f / tan(glm::angle(e_ijp, e_jjp));
  ...
  void subdivide() {
    ...
    //subdivideLinear();
    for(int i =0;i<2;i++)   subdivideLoop();

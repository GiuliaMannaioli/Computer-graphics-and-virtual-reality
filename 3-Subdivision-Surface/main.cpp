...
struct Scene {
   ...
   //  plane->render();
    ...
    void laplacianSmoothing() {
        rhino->laplacianSmoothing(0.8);
        //rhino->smoothingTaubin2(0.3, 0.35);
        rhino->init();
    }
    void cotangentlaplacianSmoothing() {
        rhino->cotanLaplacianSmoothing(0.5);
        rhino->init();
    }
    void taublinSmooth() {
        rhino->taubinSmoothing(0.3,0.35);
        rhino->init();
    }
    void Noise() {
        rhino->addNoise();
    ...
    void findParameters() {
        rhino->findOptimalSmoothingParameters();
        rhino->init();
    }
};
...
void printHelp()
{
    std::cout <<
        ...
        "    * T: Taubin smoothing" << std::endl <<
        "    * C: Cotangent laplacian smoothing" << std::endl <<
        "    * U: Laplacian smoothing" << std::endl <<
        "    * N: add noise" << std::endl <<
        "    * O: find optimal parameters" << std::endl <<
